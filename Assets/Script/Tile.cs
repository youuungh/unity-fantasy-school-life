...
public class Tile : MonoBehaviour
{
    ...
    void Update()
    {
        stepLength = Time.deltaTime * 8.0f;
        if (moveX)
            MoveX();
        else
            MoveZ();
    ...
    public void ScaleTile()
    {
        if (Mathf.Abs(distance) > 0.2f)
        {
            ...
            else
            {
                ...
                GameObject _lostPlatform = Instantiate(lostTile);
                _lostPlatform.transform.localScale = new Vector3(transform.localScale.x, transform.localScale.y, lostLength);
                _lostPlatform.transform.position = new Vector3(transform.position.x,
                    transform.position.y, transform.position.z
                    + (distance > 0 ? 1 : -1) * (transform.localScale.x - lostLength) / 2);
                _lostPlatform.GetComponent<Renderer>().material.SetColor("_Color",
                    GetComponent<Renderer>().material.GetColor("_Color"));
